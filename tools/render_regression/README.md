...
candidate-side semantic class buffer（text / dimension / hatch / geometry 等），
可用于诊断；但 AutoCAD/reference 侧没有同等 semantic mask，所以还不能把它写成
真正的 `text_iou` / `geometry_iou` pass/fail gate。当前门控指标 `ink_iou`
仍是文字+几何合并值（**故不命名 geometry_***），且文字密集金样置
`gate=false`，使字体替换期不会误判门控。补救方向：引入可信 reference semantic
mask 或明确的 semantic gate 策略后，再拆 `text_iou` + `geometry_iou`、仅几何门控。
另外 ink_iou 对**颜色**与**纵横比**回归本身盲（灰度+bbox 归一），故 compare 另出
...
`acad_manifest.json` 的每个 case 必须声明正整数 `expected_size`
（`{width,height}` 或 `[width,height]`）。缺失或非整数时 manifest validation 会 fail closed；后续
...
capture method, requested view contract, and requested expected size per row, so
operators can verify the capture/view/size contract before any returned AutoCAD
PNG exists. If a hand-written or stale request omits them, validation blocks with
`missing_requested_capture_method`, `missing_requested_view_contract`, or
`missing_requested_expected_size`; the tool never silently assumes
`plot-export`/`model-extents` and never falls back to the returned PNG's own
dimensions as the expected size. Its Markdown table also prints source DXF and
candidate PNG provenance (`sha256` and size) beside the resolved paths, so reviewers can
...
reuse guard.
If `current_acad_png` is declared but the file is not readable, request
validation emits a `current_acad_png_missing` warning because the rejected
reference sentinel could not be verified.
